...
const Header = ({ onMenuToggle }) => {
  ...
  const handleLogout = async () => {
    await logout(); // 👈 session null ho jayega
  ...
  const userInitial = <User size={16} />;
    ...
    <header className="fixed top-0 right-0 left-0 lg:left-60 h-16 bg-card border-b border-border z-200 transition-all duration-300">
      <div className="flex items-center justify-between h-full px-6">
        ...
        <div className="flex items-center space-x-2">
          {/* Notifications (Optional: can hide for guest if you want) */}
          ...
          <div className="relative">
            {session ? (
              ...
              <Button onClick={openModal} className="px-4">
                <NavLink to="/login" className="text-sm font-medium flex items-center">
